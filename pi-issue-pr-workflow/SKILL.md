...
The recheck delegation also carries this workflow's auto-resolve designation: after posting its classification replies and a verified LGTM, the reviewer either resolves the threads it classified `Resolved` (see Conversation resolution below) or hands the verified target set to the orchestrator, which then resolves them. On handoff, the reviewer's recheck report must include each target as the tuple `(thread_id, root_comment_id, reviewer_login, classification_reply_id)` (see the Review skill's recheck reporting spec), and the orchestrator treats that reported set as the target authority rather than re-deriving targets from a fresh read. The orchestrator resolves those threads with `review-threads.resolve` and confirms completion by re-reading each target with `review-threads.read` and verifying `resolved=true` (see the lightweight checks and execution below).
