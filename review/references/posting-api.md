...
再チェック分類返信の `review-comments.reply` には、helper が返した `plan_fingerprint` と、その operation の `baseline_comment_ids` を必ず渡す。利用できる場合は `baseline_comments` も渡し、body、actor、reply target、`updated_at`、`last_edited_at` の変更を Action の write preflight で検知する。Action は全ページを取得し、baseline に含まれる古い exact-body reply は dedup しない。
