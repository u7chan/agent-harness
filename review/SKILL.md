...
name: review
description: >
  GitHub Pull Request をレビューし、根拠のある指摘を差分行へ投稿する。
  PR URL・番号の指定や、「レビューして」「PRレビューして」で起動する。
---

# 概要

GitHub Pull Request をレビューし、根拠のある指摘を可能な限り差分行へ投稿する。
指摘がない場合も COMMENT review を残す。
再チェックでは以前の指摘の解消を検証し、スレッドへ返信する。
