...
assert_json_eq() {
  ...
  local expected="$3"
  local actual
  actual="$(printf '%s\n' "$json" | jq -r "$jq_filter" 2>/dev/null)"
  if [ "$actual" != "$expected" ]; then
    echo "  expected: $expected, got: $actual (filter: $jq_filter)"
    return 1
  fi
}
...
assert_json_true() {
  ...
  local actual
  local jq_status
  actual="$(printf '%s\n' "$json" | jq -e "$jq_filter" 2>/dev/null)" && jq_status=0 || jq_status=$?
  if [ "$jq_status" -ne 0 ]; then
    echo "  expected true: $jq_filter; got: $actual"
    return 1
  fi
}
