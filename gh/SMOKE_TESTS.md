...
if [ -n "$TEST_COMMENT_ID" ]; then
  jq -n --argjson reply_to "$TEST_COMMENT_ID" \
    '{number: '$TEST_PR_NUMBER', reply_to: $reply_to, body: "smoke-test-review-reply", grant: "write"}' \
...
echo "{\"number\":999999, \"reply_to\":$TEST_COMMENT_ID, \"body\": \"reply fail\", \"grant\": \"write\"}" | bash gh/scripts/gh.sh review-comments.reply 2>&1 | jq -e '.status == "failed"'
```
...
- Dedup is exact-body: when a reply with the same body, the same actor (`user.login`), and the same root (`in_reply_to_id == root`) already exists in the PR's full REST comment list, the action returns `already_applied` without POST.
- The dedup scan paginates the whole comment list; a match on any page is adopted and no write is performed.
- The write always targets the resolved root comment; a failed POST response is re-read and an exact match is adopted instead of retried (double-post guard). When no match can be adopted, the outcome is `unknown_outcome`.
- Inputs are `number`, `reply_to`, `body`, and `grant` only; no baseline IDs or thread ID is passed.
