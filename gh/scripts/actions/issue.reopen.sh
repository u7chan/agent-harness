...
main() {
  ...
  target="$(resolve_target)" || {
    envelope_fail "issue.reopen" "TARGET_ERROR" "Failed to resolve repository target" false
    exit 1
  }
  local owner_repo
  ...
  before_state="$(call_gh_api "repos/$owner_repo/issues/$number" 2>/dev/null)" || {
    envelope_fail "issue.reopen" "API_ERROR" "Failed to fetch issue" false
    exit 1
  }
  ...
  if [ "$current_state" = "open" ]; then
    envelope_already_applied "issue.reopen" "$issue_target" "$before_state"
    exit 0
  fi
  ...
  after_state="$(call_gh_api "repos/$owner_repo/issues/$number")" || {
    envelope_unknown_outcome "issue.reopen" "$issue_target" "{}"
    exit 1
  }
  ...
  if [ "$after_state_status" != "open" ]; then
    envelope_unknown_outcome "issue.reopen" "$issue_target" "$after_state"
    exit 1
  fi
  ...
  envelope_ok "issue.reopen" "$issue_target" "$formatted"
}
