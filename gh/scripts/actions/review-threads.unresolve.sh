...
main() {
  ...
  before_state="$(call_gh_api "repos/$owner_repo/pulls/comments/$thread_id" 2>"$GH_TEMP_DIR/gh-stderr")" || {
    ...
    envelope_fail "review-threads.unresolve" "API_ERROR" "Failed to fetch thread comment" false
    exit 1
  }

  local already_unresolved
  already_unresolved="$(echo "$before_state" | jq -r '.resolved // true')"
  local thread_url
  ...
  if [ "$already_unresolved" = "false" ]; then
    local already_data
    already_data="$(jq -n --argjson thread_id "$thread_id" --argjson resolved false '{thread_id: $thread_id, resolved: $resolved}')"
  ...
  after_resolved="$(echo "$after_state" | jq -r '.resolved // true')"
  ...
  local confirmation
  confirmation="$(jq -n --argjson thread_id "$thread_id" --argjson resolved false '{thread_id: $thread_id, resolved: $resolved}')"
