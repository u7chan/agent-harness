...
# review-threads.resolve and review-threads.read. The gh CLI can print error
# bodies to stdout on a zero exit, so the exit code alone is not enough.
call_graphql() {
  local query="$1"
  shift
...
fail_unverifiable_thread() {
  ...
  case "$state" in
    ...
    ok)
      ;;
    *)
      envelope_fail "review-threads.unresolve" "API_ERROR" "GraphQL node response is incomplete" false
      exit 1
      ;;
  esac
}
...
main() {
  ...
  if ! echo "$mutation_result" | jq -e --arg thread_id "$thread_id" \
    '.data.unresolveReviewThread.thread.id == $thread_id and .data.unresolveReviewThread.thread.isResolved == false' >/dev/null 2>&1; then
    envelope_unknown_outcome "review-threads.unresolve" "$thread_target" "$mutation_result"
    exit 1
  fi

  # Post-write verification: the re-read must prove that the same thread now
  # carries a real boolean false. Every unverifiable shape - fetch failure,
  ...
  if [ "$node_state" != "ok" ]; then
    envelope_unknown_outcome "review-threads.unresolve" "$thread_target" "$after_state"
    exit 1
  fi
