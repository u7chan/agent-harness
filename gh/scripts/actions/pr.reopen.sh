...
main() {
  ...
  pr_target="$(resolve_pr_target "$reference" "$number")" || {
    envelope_fail "pr.reopen" "TARGET_ERROR" "Failed to resolve PR target" false
    exit 1
  }
  ...
  before_state="$(call_gh_api "repos/$owner_repo/pulls/$pr_number" 2>/dev/null)" || {
    envelope_fail "pr.reopen" "API_ERROR" "Failed to fetch PR" false
    exit 1
  }
  ...
  if [ "$current_state" = "open" ]; then
    ...
    envelope_already_applied "pr.reopen" "$pr_target" "$formatted_before"
    exit 0
  fi
  ...
  _res="$(call_gh_api "repos/$owner_repo/pulls/$pr_number" "PATCH" --input "$body_file" 2>"$GH_TEMP_DIR/gh-stderr")" || {
    ...
    envelope_fail "pr.reopen" "API_ERROR" "Failed to reopen PR" false
    exit 1
  }
  gh_cleanup "$body_file"
  ...
  after_state="$(call_gh_api "repos/$owner_repo/pulls/$pr_number")" || {
    envelope_unknown_outcome "pr.reopen" "$pr_target" "{}"
    exit 1
  }

  local after_state_status
  after_state_status="$(echo "$after_state" | jq -r '.state')"
  if [ "$after_state_status" != "open" ]; then
    envelope_unknown_outcome "pr.reopen" "$pr_target" "$after_state"
    exit 1
  fi
  ...
  envelope_ok "pr.reopen" "$pr_target" "$formatted"
}
