...
main() {
  ...
  target="$(resolve_target)" || {
    envelope_fail "comments.update" "TARGET_ERROR" "Failed to resolve repository target" false
    exit 1
  }
  local owner_repo
  ...
  local before_state
  before_state="$(call_gh_api "repos/$owner_repo/issues/comments/$comment_id" 2>/dev/null)" || {
    envelope_fail "comments.update" "API_ERROR" "Failed to fetch comment" false
    exit 1
  }
  ...
  if cmp -s "$expected_body_file" "$GH_TEMP_DIR/before-body-tmp" 2>/dev/null; then
    ...
    envelope_already_applied "comments.update" "$comment_target" "$already_formatted"
    exit 0
  fi
  rm -f "$GH_TEMP_DIR/before-body-tmp"
  ...
  GH_RETRY_MAX=1
  local _res
  _res="$(call_gh_api "repos/$owner_repo/issues/comments/$comment_id" "PATCH" --input "$body_file" 2>"$GH_TEMP_DIR/gh-stderr")" || {
    GH_RETRY_MAX="$_saved_retry"
    gh_cleanup "$body_file"
    gh_cleanup "$expected_body_file"
    envelope_fail "comments.update" "API_ERROR" "Failed to update comment" false
    exit 1
  }
  GH_RETRY_MAX="$_saved_retry"
  gh_cleanup "$body_file"
  ...
  local after_state
  after_state="$(call_gh_api "repos/$owner_repo/issues/comments/$comment_id")" || {
    gh_cleanup "$expected_body_file"
    envelope_unknown_outcome "comments.update" "$comment_target" "{}"
    exit 1
  }
  ...
  if [ "$triple_ok" != "true" ]; then
    envelope_unknown_outcome "comments.update" "$comment_target" "$after_state"
    exit 1
  fi
  ...
  envelope_ok "comments.update" "$comment_target" "$formatted"
}
