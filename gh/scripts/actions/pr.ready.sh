...
main() {
  ...
  # Issue #171: only an open PR can be marked ready for review. A closed or
  ...
  if [ "$current_state" != "open" ]; then
    ...
    envelope_fail "pr.ready" "PR_NOT_OPEN" "PR $owner_repo#$pr_number is $state_label (draft=$current_draft); only an open PR can be marked ready for review" false
    exit 1
  fi
