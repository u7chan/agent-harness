...
- Read actions with potentially large responses (currently `pr.files.read`) bound what enters the conversation. Within the inline boundary (`GH_INLINE_MAX_BYTES`, default 20000 bytes) `data` is returned inline unchanged. Beyond it, `data` becomes `{items, truncated: true, total_count, inline_count, omitted, output_file, size_bytes}`: `items` is the lightened view with the fields named in `omitted` left out and the array capped at `GH_INLINE_MAX_ITEMS` (default 100); `output_file` holds the complete data including the omitted fields and items beyond `inline_count`. Nothing is discarded silently: read `output_file` for the parts left out and delete it when done.
