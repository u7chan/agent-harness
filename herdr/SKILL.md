...
- `granted` — an unexpired record in the delegation-scope file covers the edge, in either direction. Use `herdr/scripts/scope-grant.sh` to record, list, and revoke grants when a human authorizes an edge the shape rules do not derive; `--target-repo` grants every workspace of that repository, not one workspace ID.
